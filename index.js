...
const removeRole = () => {
	connection.query(`SELECT id,title FROM role_;`, (err, res) => {
		let roleObj = res;
		let roles = roleObj.map((role) => role.title);
		inquirer
			.prompt({
			...
			.then((answers) => {
				connection.query(
					`DELETE FROM role_ WHERE id = ${
						roleObj.find((role) => role.title == answers.roleRemove).id
					}`,
					(err, res) => {
						if (res == undefined) {
							console.log(
								`
  Could not remove ${answers.roleRemove} (make sure no one still has this role)
                    `
							);
						} else {
							console.log(`
  ${answers.roleRemove} Removed Successfully!
                  `);
						}
						init();
					}
...
const viewDepartments = () => {
	connection.query(
		`SELECT name_ AS department FROM department;`,
		(err, res) => {
			console.table(res);
			init();
		}
...
const addDepartment = () => {
	inquirer
		...
		.then((answers) => {
			connection.query(
				`INSERT INTO department (name_) VALUES ('${answers.department}')`,
				(err, res) => {
					console.log(`
			...
			.then((answers) => {
				connection.query(
					`DELETE FROM department WHERE id = ${
						depObj.find((department) => department.name_ == answers.depRemove)
							.id
					}`,
					(err, res) => {
						if (res == undefined) {
							console.log(
								`
  Could not remove ${answers.depRemove} (make sure there are no roles in this department)
                    `
							);
						} else {
							console.log(`
  ${answers.depRemove} Removed Successfully!
                  `);
						}
						init();
					}
...
const viewBudgetUsage = () => {
	connection.query(`SELECT * FROM department;`, (err, res) => {
		...
		inquirer
			...
			.then((answers) => {
				connection.query(
					`SELECT department.name_ AS department,SUM(role_.salary) AS 'budget usage' FROM employee 
                    INNER JOIN role_ ON employee.role_id = role_.id
                    INNER JOIN department ON role_.department_id = department.id AND department.name_ = "${answers.depBudget}"
                    GROUP BY department.name_;`,
					(err, res) => {
						console.table(res);
						init();
					}
